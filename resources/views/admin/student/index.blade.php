...
@section("content")
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="{{ route('student.create') }}" class="btn btn-sm btn-primary shadow-sm">
                <i class="fas fa-plus fa-sm text-white-50"></i> Create New Student Info
            </a>
        </div>
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <h6 class="m-0 font-weight-bold text-primary">Student Info Records</h6>
            </div>
            <div class="card-body">
                        ...
                        @forelse($student as $i => $item)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        @if($item->student_photo)
                                            <img src="{{ asset($item->student_photo) }}" width="30" height="30" class="rounded-circle mr-2">
                                        @endif
                                ...
                                <td class="text-center">
                                    @if($item->student_photo)
                                        <img src="{{ asset($item->student_photo) }}" width="80" class="img-thumbnail" data-toggle="modal" data-target="#imageModal" data-image="{{ asset('uploads/student_photo/' . $item->student_photo) }}" data-title="Student Photo">
                                    @else
                                ...
                                <td class="text-center">
                                    @if($item->guardian_photo)
                                        <img src=" {{ asset($item->guardian_photo) }}" width="80" class="img-thumbnail" data-toggle="modal" data-target="#imageModal" data-image="{{ asset('uploads/guardian_photo/' . $item->guardian_photo) }}" data-title="Guardian Photo">
                                    @else
                                        <span class="text-muted">N/A</span>
                                    @endif
                                </td>
                ...
                @if($student instanceof \Illuminate\Pagination\LengthAwarePaginator)
                    <div class="d-flex justify-content-between mt-3">
                        <small>Showing {{ $student->firstItem() }} to {{ $student->lastItem() }} of {{ $student->total() }} entries</small>
